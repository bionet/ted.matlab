...
for i=1:N
    for j=1:N
        G(ln2(i)+1:ln2(i+1),ln2(j)+1:ln2(j+1)) = ...
            G_block_computing(TK(1:LN(i),i)',TK(1:LN(j),j),delay(i),delay(j),bw,dt);
    end
end
...
%% Calculate and Plot MSE Over the Interval of Interest
...
text(xlimits(1)-(xlimits(2)-xlimits(1))*0.09,ylimits(1)- ...
     (ylimits(2)-ylimits(1))*0.04,'Rel. Rate:')
text(xlimits(1)-(xlimits(2)-xlimits(1))*0.12,ylimits(1)- ...
     (ylimits(2)-ylimits(1))*0.08,'# of Neurons:')
